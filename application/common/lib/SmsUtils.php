...
class SmsUtils
{
    ...
    public static function getAcsClient() {
        ...
        $accessKeyId = config('aliyun.app_key'); // AccessKeyId

        $accessKeySecret = config('aliyun.secret_key'); // AccessKeySecret
    ...
    public static function sendSms($phone = 0) {
        try {
            ...
            $request->setSignName(config('aliyun.sign_name'));

            // 必填，设置模板CODE，应严格按"模板CODE"填写, 请参考: https://dysms.console.aliyun.com/dysms.htm#/develop/template
            $request->setTemplateCode(config('aliyun.template_name'));
...
//        $request->setSmsUpExtendCode("1234567");
        ...
        if($acsResponse -> Code == 'OK'){
            //发送成功，设置失效时间
            Cache::set($phone, $code, config('aliyun.cache_time'));
            return true;
        }
        return false;
    }
     ...
     * @return bool|mixed
     */
    public static function checkSmsCache($phone = 0){
        if(!$phone){
            return false;
        }
